...
- Shelf rule (v0.3.0, from the Lab Manager rack view): ONE PC per shelf (`type: opt` = small desktop, left slot) + ONE unit on the right — a switch under test (derived item `SW-<n>`, type `dut-switch`, name from live `switch-assignments` or the OPT's JSON field `dut`) or other equipment (`equip-*`). U01 = top shelf. Outlet labels like "empty" / "KVM Power" are filtered by `NON_SERVER_LABEL`. Derived switches are not editable/movable — they follow their PC.
