...
- Camera controls: custom `MapControls` (facility-viewer style, not OrbitControls): one finger / left-drag orbits around the point touched (object or floor), two fingers / right-drag / shift-drag grab the floor plane and slide it, pinch / wheel zoom towards the point under the fingers, two-finger twist rotates. Keeps the `.target` API so `tweenCamera/flyTo/zoomBy/rotateBy/panBy` work unchanged. Tap in 3D flies only to racks; any touch cancels a running tween. Target stays inside the room, camera may hover ≤3 m outside the walls.
...
- Inventory synchronization: while connected with auto-refresh enabled, reload the same DCIM inventory endpoints every 15 seconds, including rack items, positions, labels, owners, switch assignments and chillers. Keep the last successful value when an endpoint fails; discard responses from a disconnected/replaced backend session.
