...
- Camera controls: custom `WalkControls` (not OrbitControls): one finger / left-drag = WALK in the finger's direction (up = forward, right = step right), two fingers / right-drag = turn / look, pinch / wheel = step along the view. Keeps the `.target` API so `tweenCamera/flyTo/zoomBy/rotateBy/panBy` work unchanged. Tap in 3D flies only to racks; any touch cancels a running tween.
